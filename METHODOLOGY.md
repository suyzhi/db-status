...
3. **EM258 个体校准**：`microphoneResponse` 默认 `em258NominalUncorrected`（标称频响、无个体修正、points 为空）——刻意设计：产品定位"趋势估算"，不做声级计；绝对 SPL 由耳机模型估算（±3~5 dB 量级）。要绝对校准需 94 dB 声学校准器（模型已预留 `absoluteCalibrationMode.acousticReference`）。
