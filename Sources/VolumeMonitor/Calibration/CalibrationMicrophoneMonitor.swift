...
private final class MicrophoneCaptureState: @unchecked Sendable {
    ...
    var targetLevels: [Double] = []
    var targetPeaks: [Double] = []
...
final class CalibrationMicrophoneMonitor: ObservableObject {
    ...
    func stop() {
        ...
        captureState.targetLevels.removeAll(keepingCapacity: false)
        captureState.targetPeaks.removeAll(keepingCapacity: false)
    ...
    func beginFrequencyMeasurement(_ frequencyHz: Double) {
        ...
        captureState.targetLevels.removeAll(keepingCapacity: true)
        captureState.targetPeaks.removeAll(keepingCapacity: true)
        captureState.lock.unlock()
    }
    ...
    func finishFrequencyMeasurement(
        ...
        let levels = captureState.targetLevels
        let peaks = captureState.targetPeaks
        captureState.targetFrequencyHz = nil
        captureState.targetLevels.removeAll(keepingCapacity: true)
        captureState.targetPeaks.removeAll(keepingCapacity: true)
        captureState.lock.unlock()

        guard let frequency, levels.count >= 3 else { return nil }
        let level = Self.energyMeanDB(levels)
        let stability = Self.standardDeviation(levels)
        let peak = peaks.max() ?? -96
    ...
    nonisolated private func process(_ buffer: AVAudioPCMBuffer) {
        ...
        if frequency != nil {
            captureState.targetLevels.append(Self.dbFS(strongestTarget))
            captureState.targetPeaks.append(peakDB)
            if captureState.targetLevels.count > 300 { captureState.targetLevels.removeFirst() }
            if captureState.targetPeaks.count > 300 { captureState.targetPeaks.removeFirst() }
        }
    ...
    nonisolated private static func energyMeanDB(_ values: [Double]) -> Double {
    ...
    nonisolated private static func goertzelRMS(
        samples: UnsafeBufferPointer<Float>,
        sampleRate: Double,
        frequencyHz: Double
    ) -> Double {
        guard sampleRate > 0, frequencyHz > 0, frequencyHz < sampleRate / 2 else { return 0 }
